...
def generate_speaker_activity_json(tracks, scores, threshold, output_path):
    """
    Generates a JSON file describing speaker activity segments (left/right) from tracked face data and activity scores.

    Parameters:
        tracks (list): A list of dictionaries containing face tracking data for each speaker.
        scores (list): A list of arrays, each containing activity scores for the corresponding track.
        threshold (float): The threshold above which a score is considered as speaking.
        output_path (str): The path to save the resulting speaker activity JSON file.

    Output:
        A JSON file at `output_path` containing a dict with speaker IDs as keys ("0" for right, "1" for left) 
        and a list of speech segments with "start", "end", and "position".
    """
    # Load list of extracted frame paths
    flist = glob.glob(os.path.join(args.pyframesPath, '*.jpg'))
    flist.sort()

    # Initialize storage for face data (not used in this function)
    faces = [[] for _ in range(len(flist))]

    # Load first frame to retrieve frame width
    firstImage = cv2.imread(flist[0])
    fw = firstImage.shape[1]  # Frame width

    # Initialize output dictionary for segments
    speaker_segments = {"0": [], "1": []}

    # Loop through all tracks (each corresponding to one speaker candidate)
    for idx, track in enumerate(tracks):
        frames = track["track"]["frame"]  
        x_vals = track["proc_track"]["x"] 

        bbox_list = track["track"]["bbox"] 

        x_centers = [(bbox[0] + bbox[2]) / 2 for bbox in bbox_list]
        avg_x_center = numpy.mean(x_centers)

        position = "right" if avg_x_center > fw / 2 else "left"
        speaker_id = "0" if position == "right" else "1"
        ...
        # Iterate over frames and corresponding scores (ensure same length)
        for i, frame_idx in enumerate(frames[:len(score)]):
            current_score = score[i]

            if current_score > threshold:
                count += 1
                miss = 0
                if not active and count >= 45:  # Start of a new segment
                    seg_start = frame_idx - count
                    active = True
            else:
                miss += 1
                count += 1
                if miss > 30:
                    ...
                    if active:
                        # End of a segment
                        seg_end = frame_idx - miss
                        speaker_segments[speaker_id].append({
                            "start": round(seg_start / 25.0, 2),  # Convert to seconds
                            "end": round(seg_end / 25.0, 2),
                            "position": position
                        })
                        active = False

        # If speaker was still active at the end of the track, close the segment
        if active:
            seg_end = frames[-1]
            speaker_segments[speaker_id].append({
                "start": round(seg_start / 25.0, 2),
                "end": round(seg_end / 25.0, 2),
                "position": position
            })
...
def main():
	...
	if args.evalCol == True:
		...
		json_output_path = os.path.join(args.savePath, "speaker_activity.json")
		generate_speaker_activity_json(vidTracks, scores,0.2,json_output_path)
		print(f"Speaker activity JSON saved to {json_output_path}")
